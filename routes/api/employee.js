...
router.get('/', async (req, res) => {
  try {
    ...
    const [paymentMethods] = await db.query(`
      SELECT PaymentCode, PaymentText
      FROM paymentmethod
    ...
    const [availability] = await db.query(`
      ...
      ORDER BY es.ShiftDate DESC, es.StartTime DESC
      LIMIT 25
    `);
...
router.post('/book', async (req, res) => {
  try {
    ...
    await db.query(`
      ...
      (PatientID, DoctorID, OfficeID, AppointmentDate, AppointmentTime, ReasonForVisit, StatusCode, CreatedVia)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      patientId,
      doctorId,
...
router.post('/payment', async (req, res) => {
  try {
    ...
    await db.query(`
      ...
      (AppointmentID, PatientID, PaymentCode, Amount, TransactionDateTime, Status)
      VALUES (?, ?, ?, ?, NOW(), ?)
    `, [
      appointmentId,
      patientId,
...
router.post('/availability', async (req, res) => {
  try {
    ...
    if (!employeeId || !officeId || !shiftDate || !startTime || !endTime) {
      return res.status(400).json({
        success: false,
        error: 'Missing required availability fields'
      });
    }
